...
//    self.layer.borderColor = [[UIColor grayColor] CGColor];
    ...
    self.nv= [[UIImageView alloc]initWithFrame:CGRectMake(75, 75, 150, 150)];
    self.nv.layer.cornerRadius = 75;
    self.nv.layer.masksToBounds = YES;
    [self.roundImageView addSubview:self.nv];

    
    //set play state
    UIImage *stateImage;
    if (self.isPlay) {
        stateImage = [UIImage imageNamed:@"sb"];
    }else{
        stateImage = [UIImage imageNamed:@"sb"];
    }
    self.playStateView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
